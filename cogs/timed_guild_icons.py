"""
TODO: Internal system that takes an image
TODO: Image name (filename) is parsed to retrieve data (event name and date to wait until)
TODO: Bot stores image and date
TODO: Bot waits until date
TODO: Bot changes server icon to image tied to date
TODO: This is an "event" (and will be announced somewhere:tm: [my idea :animmLul:])
"""
import asyncio
import calendar
import re
...
from typing import Awaitable, Callable, NamedTuple, Tuple, TypeAlias, Any

from discord.ext import tasks

from bot import HideoutManager
from utils import HideoutCog, HideoutManagerException, DUCK_HIDEOUT
...
class FileNameParsingFailure(HideoutManagerException):
    """Raised when parsing a file name fails."""


class EventNotFound(HideoutManagerException):
...
class EventsManager:
    ...
    async def parse_filename(self, file: Path) -> FileInfo | None:
        ...
        # TODO: document what it raises, etc.
        # TODO: make this into a cog for shared state.
        # TODO: make another function that generates an abstract calendar for each year on cog load / new year's.
        # TODO: make a system to handle special-cased files from the assets folder itself, rather than a callable dict.
        # TODO: impl actual folder parsing.
        # TODO: create github PR checks/CI for verifying file names and special case functions.
        """
        filename = file.name
        try:
            ...
            elif match := SPECIAL_CASE_PATTERN.fullmatch(filename):  # Special-cased callables that require custom state.
                special_case_id: str = match.group('CID')

                module = __import__(f'assets.guild_icons.special_cases.{special_case_id}')
    ...
    async def daily_task(self):
        ...
        try:
            ...
            with event.file.open('rb') as fp:
                print(f'Would have edited guild with file {event.file.name}')
            ...
            with file.open('rb') as fp:
                print(f'Would have edited guild with file {file.name}')
                if not self.bot.no_automatic_features:
                    await guild.edit(icon=fp.read())
