    ...
    if isinstance(error, app_commands.CommandNotFound):
        if not response._responded:  # noqa
            await response.send_message(
                ...
                'and we are working hard to make them better.',
                ephemeral=True,
            )
        ...
        embed.set_footer(
            text='This error has bee succesfully reported to the developers.',
            icon_url='https://cdn.discordapp.com/emojis/912190496791728148.gif?size=60&quality=lossless',
        )
        msg = (
            '**Sorry, but something went wrong while executing this application command.**\n'
