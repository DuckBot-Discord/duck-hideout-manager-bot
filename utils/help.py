...
def _backup_command_embed(command: commands.Command) -> discord.Embed:
    ...
    embed.add_field(name='How to use', value=f'`db.{command.qualified_name} {command.signature}'.strip() + '`')

    if subcommands := getattr(command, 'commands', None):
        embed.add_field(
            name='Subcommands', value=human_join([f'`{c.name}`' for c in subcommands], final='and'), inline=False
        )
...
class HelpGroup(discord.ui.View):
    ...
    def __init__(
        self, *, parent: ParentType, group: DuckGroup, author: Optional[Union[discord.Member, discord.User]] = None
    ) -> None:
        super().__init__()
...
class HelpCommand(discord.ui.View):
    ...
    def __init__(
        self, *, parent: ParentType, command: DuckCommand, author: Optional[Union[discord.Member, discord.User]] = None
    ) -> None:
        super().__init__()
...
class HelpCog(discord.ui.View):
    ...
    def __init__(
        self, *, parent: ParentType, cog: DuckCog, author: Optional[Union[discord.Member, discord.User]] = None
    ) -> None:
        super().__init__()
...
class HelpView(discord.ui.View):
    ...
    __slots__: Tuple[str, ...] = ('parent', 'bot', 'author', 'interaction_check', '_cs_embed')

    def __init__(
        self, /, *, parent: ParentType, cogs: List[DuckCog], author: Optional[Union[discord.Member, discord.User]] = None
    ) -> None:
        super().__init__()
        self.parent: ParentType = parent
        self.bot: DuckBot = _find_bot(parent)
